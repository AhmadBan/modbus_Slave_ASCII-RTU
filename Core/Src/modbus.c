...
uint8_t execute_modbus_command(uint8_t *buffer, uint8_t size)
{
	...
	case 5:
	{
		sendIndex = ResponseForceSingleCoil_05(buffer, size,mb);
		break;
	}
	case 6:
	{
		sendIndex = ResponsePresetSingleRegister_06(buffer, size,mb);
